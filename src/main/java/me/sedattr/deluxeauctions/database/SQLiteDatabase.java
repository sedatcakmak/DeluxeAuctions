...
public class SQLiteDatabase implements DatabaseManager {
    ...
    public void saveAuctions() {
        ...
        runTask(() -> {
            ...
            // runs on the main thread: one commit instead of one fsync-per-row makes the
            // shutdown flush dramatically shorter. During normal async runtime saves we stay
            // on autocommit so a background thread never flips the shared connection's mode.
            ...
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                ...
                Logger.sendConsoleMessage("&f" + saved + " %level_color%auctions saved in &f" + (System.currentTimeMillis()-time) + " ms%level_color%!"
